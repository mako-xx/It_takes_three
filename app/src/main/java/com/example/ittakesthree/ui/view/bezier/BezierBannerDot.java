package com.example.ittakesthree.ui.view.bezier;
...
public class BezierBannerDot extends View implements ViewPager.OnPageChangeListener {
    ...
    public BezierBannerDot(Context context, int selectColor, int unSelectedColor) {
        super(context);
    ...
    private void initattrs(AttributeSet attrs) {
        ...
        mSelectedColor = typedArray.getColor(R.styleable.BezierBannerDot_selectedColor, this.mSelectedColor);
        mUnSelectedColor = typedArray.getColor(R.styleable.BezierBannerDot_unSelectedColor, this.mUnSelectedColor);
    ...
    public void setColor(int selectColor,int unSelectedColor){
        this.mSelectedColor=selectColor;
        this.mUnSelectedColor=unSelectedColor;
    }
    public void setRadius(int selectRadius,int unSelectRadius){
        this.mRadius=selectRadius;
        this.mNomarlRadius=unSelectRadius;
    }
        ...
        for (int i = 0; i < count; i++) {
            if (mDrection == DIRECTION_RIGHT) {
                if (i == mSelectedIndex || i == mSelectedIndex + 1) {
                    ...
                    RectF oval1 = new RectF(getCenterPointAt(i) - mNomarlRadius, mRadius - CommonTool.dp2px(1.5f), getCenterPointAt(i) + mNomarlRadius, mRadius + CommonTool.dp2px(1.5f));// 设置个新的长方形
                    canvas.drawRoundRect(oval1, 5, 2, mCirclePaint2);//第二个参数是x半径，第三个参数是y半径
...
//                    canvas.drawCircle(getCenterPointAt(i), mRadius, mNomarlRadius, mCirclePaint2);
        ...
        RectF oval1 = new RectF(mSupport_next_centerX - mSupport_Next_ChangeRadius, mSupport_next_centerY - CommonTool.dp2px(1.5f), mSupport_next_centerX + mSupport_Next_ChangeRadius, mSupport_next_centerY + CommonTool.dp2px(1.5f));// 设置个新的长方形
        canvas.drawRoundRect(oval1, 5, 2, mCirclePaint2);//第二个参数是x半径，第三个参数是y半径
        RectF oval2 = new RectF(mbgNextPointX - mChangeBgRadius, mbgNextPointY - CommonTool.dp2px(1.5f), mbgNextPointX + mChangeBgRadius, mbgNextPointY + CommonTool.dp2px(1.5f));// 设置个新的长方形
        canvas.drawRoundRect(oval2, 5, 2, mCirclePaint2);//第二个参数是x半径，第三个参数是y半径
...
//        canvas.drawCircle(mbgNextPointX, mbgNextPointY, mChangeBgRadius, mCirclePaint2);
        ...
        RectF oval3 = new RectF(mSupportCircleX - mSupportChangeRadius, mSupportCircleY - CommonTool.dp2px(1.5f), mSupportCircleX + mSupportChangeRadius, mSupportCircleY + CommonTool.dp2px(1.5f));// 设置个新的长方形
        canvas.drawRoundRect(oval3, 5, 2, mCirclePaint);//第二个参数是x半径，第三个参数是y半径
        RectF oval4 = new RectF(mCenterPointX - mChangeRadius, mCenterPointY - CommonTool.dp2px(1.5f), mCenterPointX + mChangeRadius, mCenterPointY + CommonTool.dp2px(1.5f));// 设置个新的长方形
        canvas.drawRoundRect(oval4, 5, 2, mCirclePaint);//第二个参数是x半径，第三个参数是y半径
...
//        float msupportradianY = (float) (Math.cos(supportradian) * mSupportChangeRadius);
        float msupportradianY =CommonTool.dp2px(1.5f);

        //起点
        mStartX = mCenterPointX + mX;
        mStartY = mCenterPointY - CommonTool.dp2px(1.5f);
        ...
        mPath.quadTo(controlPointX, mRadius, mStartX, mStartY + 2 * CommonTool.dp2px(1.5f));
...
//        float mY_next = (float) (Math.cos(m_Next_radian) * mChangeBgRadius);
        float mY_next = CommonTool.dp2px(1.5f);
        //辅助圆圆心
...
//        float mSupport_Next_radianY = (float) (Math.cos(mSupport_Next_radian) * mSupport_Next_ChangeRadius);
        float mSupport_Next_radianY =  CommonTool.dp2px(1.5f);
    ...
    private void moveToPrivous() {
        ...
        //Y轴距离圆心距离
//        float mY = (float) (Math.cos(radian) * mChangeRadius);
        float mY = CommonTool.dp2px(1.5f);
